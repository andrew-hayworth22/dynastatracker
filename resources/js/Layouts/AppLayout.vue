...
const navigation = [
    {
        name: 'Dashboard',
        url: route('dynasties.index'),
        icon: HomeIcon,
        current: route().current('dynasties.index')
    },
]
                            ...
                            <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-gray-900 px-6 pb-4 ring-1 ring-white/10">
                                ...
                                <nav class="flex flex-1 flex-col">
                                    <ul role="list" class="flex flex-1 flex-col gap-y-7">
                                        ...
                                        <li>
                                            <div class="text-xs font-semibold leading-6 text-gray-400">Latest dynasties</div>
                            ...
                            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                                <MenuItems class="absolute right-0 z-10 mt-2.5 w-32 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                                    <MenuItem v-for="item in userNavigation" :key="item.name">
                                        <Link as="button" :href="item.url" :class="[item.current ? 'bg-gray-50' : '', 'block px-3 py-1 text-sm leading-6 text-gray-900']">{{ item.name }}</Link>
                                    </MenuItem>
                                    <form method="POST" @submit.prevent="logout">
                                        <MenuItem as="button" class="block px-3 py-1 text-sm leading-6 text-gray-900">Sign out</MenuItem>
                                    </form>
                                </MenuItems>
                            </transition>
